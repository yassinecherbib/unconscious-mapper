...
services/season_detector.py (new file)
...
periods of relative stability or stagnation, punctuated by inflection points.
Triggering the longitudinal analyzer at those inflection points is what makes
the output meaningful rather than noise.
...
def should_trigger_longitudinal(
    ...
    Returns:
        {
            "should_trigger": bool,
            "reasons": list[str],   # which conditions fired
    ...
    if len(entries) > ARCHETYPE_ROTATION_WINDOW * 2:
        # Find dominant archetype in first half
        ...
        if early_archetypes:
            ...
            # Check if it's absent from the last N entries
